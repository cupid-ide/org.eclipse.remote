...
public class JSchConnection implements IRemoteConnection {
	...
	private Session newSession(final IUserAuthenticator authenticator, IProgressMonitor monitor) throws RemoteConnectionException {
		...
		try {
			...
			final IRemoteConnection proxyConnection = getProxyConnection();
			if (!getProxyCommand().equals(EMPTY_STRING)) {
				session.setProxy(JSchConnectionProxyFactory.createCommandProxy(proxyConnection, getProxyCommand(),
						progress.newChild(10)));
				session.connect(getTimeout() * 1000); // the fJSchService doesn't pass the timeout correctly
			} else if (proxyConnection instanceof JSchConnection) {
				session.setProxy(JSchConnectionProxyFactory.createForwardProxy((JSchConnection) proxyConnection,
						progress.newChild(10)));
