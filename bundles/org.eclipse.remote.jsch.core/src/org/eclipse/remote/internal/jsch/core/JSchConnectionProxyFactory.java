...
public class JSchConnectionProxyFactory {
	private static class CommandProxy implements Proxy {
		...
		public void connect(SocketFactory socket_factory, String host,
			...
			boolean bOutputAvailble, bProcessComplete, bTimedOut, bCanceled;
			do {
				try {
	...
	private static class SSHForwardProxy implements Proxy {
		...
		private SSHForwardProxy(JSchConnection connection, IProgressMonitor monitor) {
			this.connection = connection;
			this.monitor = monitor;
		}
	 ...
	 * @return ssh proxy
	 */
	public static Proxy createForwardProxy(JSchConnection connection, IProgressMonitor monitor) {
		return new SSHForwardProxy(connection, monitor);
	}
}
